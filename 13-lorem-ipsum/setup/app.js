...
form.addEventListener("submit", function(e) {
  // Prevent the page to send the data to the server (in order to see the workings of our project).
  e.preventDefault();
  ...
  if(isNaN(value) || value <= 0) {
    ...
    for (let i = 0; i < value; i++) {
      let localRandom = Math.floor(Math.random() * text.length);
      let randomPara = `<p class="result">${text[localRandom]}</p>`;
      resultText = resultText + randomPara;
    }
